 ...
 * Cutting corners
 * ===============
 * In some instances this code will produce the incorrect results. The
 * elliptical point addition does for example not properly handle the case of
 * two points sharing the same X-coordinate. The reason the code doesn't handle
 * it properly is because it is very unlikely to ever occur and the performance
 * penalty for doing it right is too severe. In the future I'll introduce a
 * periodic check after N amount of cycles that verifies the integrity of all
 * the points to make sure that even very unlikely event are at some point
 * rectified.
 *
...
__constant const mp_number mod = {{0xfffffc2f, 0xfffffffe, 0xffffffff,
                                   0xffffffff, 0xffffffff, 0xffffffff,
                                   0xffffffff, 0xffffffff}};

// tripleNegativeGx =
// 0x92c4cc831269ccfaff1ed83e946adeeaf82c096e76958573f2287becbb17b196
__constant const mp_number tripleNegativeGx = {
    {0xbb17b196, 0xf2287bec, 0x76958573, 0xf82c096e, 0x946adeea, 0xff1ed83e,
     0x1269ccfa, 0x92c4cc83}};

// doubleNegativeGy =
// 0x6f8a4b11b2b8773544b60807e3ddeeae05d0976eb2f557ccc7705edf09de52bf
__constant const mp_number doubleNegativeGy = {
    {0x09de52bf, 0xc7705edf, 0xb2f557cc, 0x05d0976e, 0xe3ddeeae, 0x44b60807,
     0xb2b87735, 0x6f8a4b11}};

// negativeGy       =
// 0xb7c52588d95c3b9aa25b0403f1eef75702e84bb7597aabe663b82f6f04ef2777
__constant const mp_number negativeGy = {{0x04ef2777, 0x63b82f6f, 0x597aabe6,
                                          0x02e84bb7, 0xf1eef757, 0xa25b0403,
                                          0xd95c3b9a, 0xb7c52588}};

// Multiprecision subtraction. Underflow signalled via return value.
mp_word mp_sub(mp_number *const r, const mp_number *const a,
               const mp_number *const b) {
  mp_word t, c = 0;
...
// Multiprecision subtraction of the modulus saved in mod. Underflow signalled
// via return value.
mp_word mp_sub_mod(mp_number *const r) {
  mp_number mod = {{0xfffffc2f, 0xfffffffe, 0xffffffff, 0xffffffff, 0xffffffff,
                    0xffffffff, 0xffffffff, 0xffffffff}};
...
// number. I've chosen to do this because:
//   1. It's easier to re-use an already existing function
//   2. A modular addition would have more overhead since it has to determine if
//   the result of
//      the addition (r) is in the gap M <= r < 2^256. This overhead doesn't
//      exist in a subtraction. We immediately know at the end of a subtraction
//      if we had underflow or not by inspecting the carry value. M refers to
//      the modulus saved in variable mod.
void mp_mod_sub(mp_number *const r, const mp_number *const a,
                const mp_number *const b) {
  mp_word i, t, c = 0;
...
// I made this in the belief that using constant address space instead of
// private address space for any constant numbers would lead to increase in
// performance. Judges are still out on this one.
void mp_mod_sub_const(mp_number *const r, __constant const mp_number *const a,
                      const mp_number *const b) {
  mp_word i, t, c = 0;
...
void mp_mod_sub_gx(mp_number *const r, const mp_number *const a) {
  ...
  t = a->d[0] - 0x16f81798;
  c = t < a->d[0] ? 0 : (t == a->d[0] ? c : 1);
  r->d[0] = t;
  t = a->d[1] - 0x59f2815b - c;
  c = t < a->d[1] ? 0 : (t == a->d[1] ? c : 1);
  r->d[1] = t;
  t = a->d[2] - 0x2dce28d9 - c;
  c = t < a->d[2] ? 0 : (t == a->d[2] ? c : 1);
  r->d[2] = t;
  t = a->d[3] - 0x029bfcdb - c;
  c = t < a->d[3] ? 0 : (t == a->d[3] ? c : 1);
  r->d[3] = t;
  t = a->d[4] - 0xce870b07 - c;
  c = t < a->d[4] ? 0 : (t == a->d[4] ? c : 1);
  r->d[4] = t;
  t = a->d[5] - 0x55a06295 - c;
  c = t < a->d[5] ? 0 : (t == a->d[5] ? c : 1);
  r->d[5] = t;
  t = a->d[6] - 0xf9dcbbac - c;
  c = t < a->d[6] ? 0 : (t == a->d[6] ? c : 1);
  r->d[6] = t;
  t = a->d[7] - 0x79be667e - c;
  c = t < a->d[7] ? 0 : (t == a->d[7] ? c : 1);
  r->d[7] = t;
...
void mp_mod_sub_gy(mp_number *const r, const mp_number *const a) {
  ...
  c = t < a->d[0] ? 0 : (t == a->d[0] ? c : 1);
  r->d[0] = t;
  t = a->d[1] - 0x9c47d08f - c;
  c = t < a->d[1] ? 0 : (t == a->d[1] ? c : 1);
  r->d[1] = t;
  t = a->d[2] - 0xa6855419 - c;
  c = t < a->d[2] ? 0 : (t == a->d[2] ? c : 1);
  r->d[2] = t;
  t = a->d[3] - 0xfd17b448 - c;
  c = t < a->d[3] ? 0 : (t == a->d[3] ? c : 1);
  r->d[3] = t;
  t = a->d[4] - 0x0e1108a8 - c;
  c = t < a->d[4] ? 0 : (t == a->d[4] ? c : 1);
  r->d[4] = t;
  t = a->d[5] - 0x5da4fbfc - c;
  c = t < a->d[5] ? 0 : (t == a->d[5] ? c : 1);
  r->d[5] = t;
  t = a->d[6] - 0x26a3c465 - c;
  c = t < a->d[6] ? 0 : (t == a->d[6] ? c : 1);
  r->d[6] = t;
  t = a->d[7] - 0x483ada77 - c;
  c = t < a->d[7] ? 0 : (t == a->d[7] ? c : 1);
  r->d[7] = t;
...
// Multiprecision addition of the modulus saved in mod. Overflow signalled via
// return value.
...
// Multiprecision addition of two numbers with one extra word each. Overflow
// signalled via return value.
mp_word mp_add_more(mp_number *const r, mp_word *const extraR,
                    const mp_number *const a, const mp_word *const extraA) {
...
// Multiplies a number with a word and adds it to an existing number with an
// extra word, overflow of the extra word is signalled in return value This is a
// special function only used for modular multiplication
mp_word mp_mul_word_add_extra(mp_number *const r, const mp_number *const a,
                              const mp_word w, mp_word *const extra) {
...
// Multiplies a number with a word, potentially adds modhigher to it, and then
// subtracts it from en existing number, no extra words, no overflow This is a
// special function only used for modular multiplication
void mp_mul_mod_word_sub(mp_number *const r, const mp_word w,
                         const bool withModHigher) {
  // Having these numbers declared here instead of using the global values in
  // __constant address space seems to lead to better optimizations by the
  // compiler on my GTX 1070.
  mp_number mod = {{0xfffffc2f, 0xfffffffe, 0xffffffff, 0xffffffff, 0xffffffff,
                    0xffffffff, 0xffffffff, 0xffffffff}};
  mp_number modhigher = {{0x00000000, 0xfffffc2f, 0xfffffffe, 0xffffffff,
  ...
  for (mp_word i = 0; i < MP_WORDS; ++i) {
    ...
    cA = tM < (withModHigher ? modhigher.d[i] : 0)
             ? 1
             : (tM == (withModHigher ? modhigher.d[i] : 0) ? cA : 0);
...
// Modular multiplication. Based on Algorithm 3 (and a series of hunches) from
// this article:
// https://www.esat.kuleuven.be/cosic/publications/article-1191.pdf
// When I first implemented it I never encountered a situation where the
// additional end steps of adding or subtracting the modulo was necessary. Maybe
// it's not for the particular modulo used in secp256k1, maybe the overflow bit
// can be skipped in to avoid 8 subtractions and trade it for the final steps?
// Maybe the final steps are necessary but seldom needed? I have no idea, for
// the time being I'll leave it like this, also see the comments at the
// beginning of this document under the title "Cutting corners".
void mp_mod_mul(mp_number *const r, const mp_number *const X,
                const mp_number *const Y) {
  mp_number Z = {{0}};
  mp_word extraWord;
...
void mp_mod_inverse(mp_number *const r) {
  ...
  while (r->d[0] || r->d[1] || r->d[2] || r->d[3] || r->d[4] || r->d[5] ||
         r->d[6] || r->d[7]) {
    while (!(r->d[0] & 1)) {
      mp_shr(r);
      if (A.d[0] & 1) {
...
// Does not handle points sharing X coordinate, this is a deliberate design
// choice. For more information on this choice see the beginning of this file.
...
void profanity_init_seed(__global const point *const precomp, point *const p,
                         bool *const pIsFirst, const size_t precompOffset,
                         const ulong seed) {
  point o;
...
__kernel void profanity_init(__global const point *const precomp,
                             __global mp_number *const pDeltaX,
                             __global mp_number *const pPrevLambda,
                             __global result *const pResult, const ulong4 seed,
                             const ulong4 seedX, const ulong4 seedY) {
  ...
  // by the private key represented in seed)
...
//
// My RX 480 is very sensitive to changes in the second loop and sometimes I
// have to make seemingly non-functional changes to the code to make the
// compiler generate the most optimized version.
__kernel void profanity_inverse(__global const mp_number *const pDeltaX,
                                __global mp_number *const pInverse) {
  const size_t id = get_global_id(0) * PROFANITY_INVERSE_SIZE;

  // negativeDoubleGy =
  // 0x6f8a4b11b2b8773544b60807e3ddeeae05d0976eb2f557ccc7705edf09de52bf
  mp_number negativeDoubleGy = {{0x09de52bf, 0xc7705edf, 0xb2f557cc, 0x05d0976e,
                                 0xe3ddeeae, 0x44b60807, 0xb2b87735,
                                 0x6f8a4b11}};
  ...
  // buffer[i] = pDeltaX[id] * pDeltaX[id + 1] * pDeltaX[id + 2] * ... *
  // pDeltaX[id + i] buffer2[i] = pDeltaX[id + i]
  buffer[0] = pDeltaX[id];
...
__kernel void profanity_iterate(__global mp_number *const pDeltaX,
                                __global mp_number *const pInverse,
                                __global mp_number *const pPrevLambda) {
  const size_t id = get_global_id(0);

  // negativeGx =
  // 0x8641998106234453aa5f9d6a3178f4f8fd640324d231d726a60d7ea3e907e497
  mp_number negativeGx = {{0xe907e497, 0xa60d7ea3, 0xd231d726, 0xfd640324,
                           0x3178f4f8, 0xaa5f9d6a, 0x06234453, 0x86419981}};
  ...
  // λ' = - (2G_y) / d' - λ <=> lambda := pInversedNegativeDoubleGy[id] -
  // pPrevLambda[id]
  ...
  // Save public address hash in pInverse, only used as interim storage until
  // next cycle
...
void profanity_result_update(const size_t id, __global const uchar *const hash,
                             __global result *const pResult, const uchar score,
                             const uchar scoreMin,
                             __global uint *const pResultIndex) {
  if (score >= scoreMin) {
    uint index = atomic_inc(pResultIndex);
    ...
    for (int i = 0; i < 20; ++i) {
      pResult[index].foundHash[i] = hash[i];
    }
  }
}
...
__kernel void profanity_score_benchmark(__global mp_number *const pInverse,
  ...
  profanity_result_update(id, hash, pResult, score, scoreMin, pResultIndex);
}

__kernel void profanity_score_matching(__global mp_number *const pInverse,
                                       __global result *const pResult,
                                       __constant const uchar *const data1,
                                       __constant const uchar *const data2,
                                       const uchar scoreMin,
...
__kernel void profanity_score_leading(__global mp_number *const pInverse,
                                      __global result *const pResult,
                                      __constant const uchar *const data1,
                                      __constant const uchar *const data2,
                                      const uchar scoreMin,
...
__kernel void profanity_score_range(__global mp_number *const pInverse,
                                    __global result *const pResult,
                                    __constant const uchar *const data1,
                                    __constant const uchar *const data2,
                                    const uchar scoreMin,
...
__kernel void profanity_score_zerobytes(__global mp_number *const pInverse,
                                        __global result *const pResult,
                                        __constant const uchar *const data1,
                                        __constant const uchar *const data2,
                                        const uchar scoreMin,
...
__kernel void profanity_score_leadingrange(__global mp_number *const pInverse,
                                           __global result *const pResult,
                                           __constant const uchar *const data1,
                                           __constant const uchar *const data2,
                                           const uchar scoreMin,
...
__kernel void profanity_score_mirror(__global mp_number *const pInverse,
                                     __global result *const pResult,
                                     __constant const uchar *const data1,
                                     __constant const uchar *const data2,
                                     const uchar scoreMin,
...
__kernel void profanity_score_doubles(__global mp_number *const pInverse,
                                      __global result *const pResult,
                                      __constant const uchar *const data1,
                                      __constant const uchar *const data2,
                                      const uchar scoreMin,
  ...
  for (int i = 0; i < 20; ++i) {
    if ((hash[i] == 0x00) || (hash[i] == 0x11) || (hash[i] == 0x22) ||
        (hash[i] == 0x33) || (hash[i] == 0x44) || (hash[i] == 0x55) ||
        (hash[i] == 0x66) || (hash[i] == 0x77) || (hash[i] == 0x88) ||
        (hash[i] == 0x99) || (hash[i] == 0xAA) || (hash[i] == 0xBB) ||
        (hash[i] == 0xCC) || (hash[i] == 0xDD) || (hash[i] == 0xEE) ||
        (hash[i] == 0xFF)) {
      ++score;
    } else {
      break;
